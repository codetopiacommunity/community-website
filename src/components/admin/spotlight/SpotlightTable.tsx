...
export function SpotlightTable({ spotlights, loading, onEdit, onDelete, onAddFirst, onRefetch }: SpotlightTableProps) {
  ...
  const handleFeature = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) { toast.success("Spotlight featured"); onRefetch(); }
    ...
    <div className="bg-white border border-zinc-200 overflow-hidden">
      ...
      {!loading && spotlights.length === 0 && (
        <div className="py-16 text-center">
          ...
          <p className="font-mono text-sm font-semibold text-zinc-900">No spotlights yet</p>
                ...
                <tr key={s.id} className="hover:bg-zinc-50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-mono font-semibold text-sm text-zinc-900">{s.name}</p>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    {s.featured ? (
                      ...
                      >
                        {featuringId === s.id ? <Loader2 className="h-2.5 w-2.5 animate-spin" /> : <Star className="h-2.5 w-2.5" />}
                        Set Featured
                      </button>
                    )}
                  </td>
