  ...
  return (
    <section className="w-full py-20 md:py-24 bg-black flex flex-col border-t border-zinc-900 overflow-hidden">
      <Container className="w-full font-sans">
        ...
        */}
        <div className="grid grid-cols-1 lg:grid-cols-3 lg:h-[300px] border border-zinc-800 bg-zinc-900 gap-px overflow-hidden group">
          <div className="relative h-64 sm:h-80 lg:h-full overflow-hidden bg-zinc-950">
            <Image
          ...
          <div className="lg:col-span-2 flex flex-col justify-center gap-6 bg-black p-8 md:p-10">
            ...
            <div className="flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-zinc-800 pt-6">
              {/* The feature's own page: what the person can actually share. */}
              <Link
                href={`/spotlight/${spotlightHandle(spotlight)}`}
                className="group/feature inline-flex items-center gap-2 text-white font-mono text-xs uppercase tracking-widest hover:text-zinc-400 transition-colors duration-200"
              >
                Full feature
                <ArrowRight className="w-4 h-4 transition-transform group-hover/feature:translate-x-0.5 motion-reduce:transition-none" />
              </Link>
